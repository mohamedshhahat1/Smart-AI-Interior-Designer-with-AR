...
class _RoomScanScreenState extends State<RoomScanScreen> {
  ...
  Future<void> _analyzeRoom() async {
    ...
    try {
      final room = await _apiService.uploadRoom(_selectedImage!);
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Failed to analyze room. Please try again.')),
        );
      }
    } finally {
  ...
  Widget _buildCaptureOptions() {
    return Column(
      children: [
        ...
        Text('Capture or select a room photo', style: TextStyle(fontSize: 18, color: Colors.grey[600])),
  ...
  Widget _buildStyleSelector() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Choose Design Style', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
        const SizedBox(height: 12),
        Wrap(
          spacing: 8,
          runSpacing: 8,
          children: AppConstants.supportedStyles.map((style) {
            ...
            return ChoiceChip(
              label: Text(style),
              selected: isSelected,
              onSelected: (selected) => setState(() => _selectedStyle = selected ? style : null),
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        ...
        child: _isAnalyzing
            ...
            : const Text('Analyze & Generate Design', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600)),
      ),
    );
  }
}
